...
| Kind | Use it when |
| --- | --- |
| `profile.email-match` | A profile page itself shows the address we hold. Decisive. |
...
observation, not two: a profile whose name and title both match is one
`profile.email-match`, not a name match plus a title match. Splitting it would
double-count a single page into false certainty, which is exactly the
