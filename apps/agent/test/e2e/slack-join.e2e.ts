...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  Janus joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: Janus is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to Janus",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets Janus add itself to a private channel"
			: "no user token, so Janus cannot add itself to a private channel",
	);
