...
export type CrmCache = {
	...
	forms(id?: string, options?: Options): Promise<void>;
	recents(options?: Options): Promise<void>;
	...
	return {
		fields: (entity, options) =>
			run(
				...
				[
					trpc.templates.mergeFields.queryKey(),
					...(entity
		...
		pipeline: (options) =>
			run(
				...
				[
					...
					trpc.dashboard.pipelineStages.queryKey(),
				],
				options,
			),
		...
		forms: (id, options) =>
			run(
				[id ? trpc.forms.byId.queryKey({ id }) : trpc.forms.byId.queryKey()],
				[trpc.forms.list.queryKey(), trpc.forms.submissions.pathKey()],
				options,
			),

		recents: (options) => run([trpc.recents.list.queryKey()], [], options),
