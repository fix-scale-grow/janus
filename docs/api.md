...
- **`sso.signInOptions`, `contractSigning.*` and `proposalView.*` are the
  app's only public procedures.** Every other `sso.*` takes `AuthMiddleware`
  at the *method*, which is what leaves it open. A client secret is never
  read back out.
